...
class Unet(nn.Module):
    def __init__(
        ...
        self.dims = [self.channels] + dims
        self.device = device

        self.with_time_emb = with_time_emb

        if self.with_time_emb:
            self.time_dim = self.image_dim * 4  # why this?
            self.time_mlp = nn.Sequential(
                SinusoidalPosEmb(dim=self.image_dim),
                nn.Linear(in_features=self.image_dim, out_features=self.time_dim),
                nn.GELU(),
                nn.Linear(in_features=self.time_dim, out_features=self.time_dim),
            )
        else:
            self.time_emb = None
            self.time_mlp = None

        self.encoder = Encoder(dims=self.dims, time_emb_dim=self.time_dim)

        self.bottleneck = ResNetBlock(
            dim=self.dims[-2], dim_out=self.dims[-1], time_emb_dim=self.time_dim
        )
        self.bottleneck_attn = Residual(PreNorm(dims[-1], Attention(self.dims[-1])))

        self.decoder = Decoder(
            dims=list(reversed(self.dims)), time_emb_dim=self.time_dim
        )

        self.final_conv = nn.Sequential(
            ResNetBlock(dim=self.image_dim, dim_out=self.image_dim),
            nn.Conv2d(in_channels=self.image_dim, out_channels=out_dim, kernel_size=1),
        )
